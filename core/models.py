...
class CategoriaVideo(models.Model):
    ...
    pasta = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="Nome da pasta no Cloudflare R2",
    )
...
class MusicaBase(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.arquivo_musica:
            ...
            self.object_key = (
                f"media/musicas_base/{self.categoria.pasta}/{nome_arquivo}"
            )
        super().save(*args, **kwargs)
...
class VideoGerado(models.Model):
    ...
    arquivo_final = models.CharField(
        max_length=500,
        blank=True,
        null=True,
        help_text="Caminho do vídeo gerado (local ou R2)",
    )
    ...
    caminho_audio_narrador = models.CharField(
        max_length=500,
        blank=True,
        null=True,
        help_text="Caminho do áudio de narração gerado",
    )
    caminho_legenda_ass = models.CharField(
        max_length=500, blank=True, null=True, help_text="Caminho da legenda ASS gerada"
    )
    caminho_imagem_texto = models.CharField(
        max_length=500,
        blank=True,
        null=True,
...
class Assinatura(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.usuario.username} - {self.plano.nome} ({self.get_status_display()})"
        )
